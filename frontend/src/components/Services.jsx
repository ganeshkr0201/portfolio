...
const services = [
  {
    ...
    icon: <Server size={32} className="text-neon" />,
  ...
  },
  {
    ...
    icon: <Code2 size={32} className="text-neon" />,
...
const Services = () => {
  return (
    <section id="services" className="w-full py-24 px-4 sm:px-6 lg:px-8 relative">
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-neon/10 rounded-full blur-[150px] pointer-events-none -translate-y-1/2"></div>
        ...
        >
          {services.map((service, idx) => (
            <motion.div 
              ...
              className="glass-card p-8 group relative overflow-hidden flex flex-col h-full bg-white/[0.03] hover:bg-white/[0.05]"
            >
              <div className="flex items-center gap-6 mb-6">
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 group-hover:bg-gradient-to-br group-hover:from-neon/20 group-hover:to-accent/20 transition-all duration-300 transform group-hover:-translate-y-1">
                  {service.icon}
                </div>
              ...
              <div className="space-y-3 pt-6 border-t border-white/10 w-full mt-auto">
                {service.features.map((feature, fIdx) => (
                  <div key={fIdx} className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-neon to-accent"></div>
